...
import { FeatureFlagDataSet } from "@/types/feature-flags";
...
> = (props) => {
    ...
    const [featureFlagSet, setFeatureFlagSet] = useState<FeatureFlagDataSet>(
        currentFeatureFlagSet
    );

    const value = useMemo(() => ({ featureFlagSet }), [featureFlagSet]);
    return (
...
export const FeatureFlagConsumer: FC<FeatureFlagConsumerProps> = ({
    children,
}) => {
    return (
