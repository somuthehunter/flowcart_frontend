...
> {
    ...
    ) => void; // More flexible type for setter
    joinOperator: JoinOperator;
    setJoinOperator: (
        value: JoinOperator | ((prev: JoinOperator) => JoinOperator)
    ) => void; // More flexible type for setter
    debounceMs?: number;
    canAddMultipleFilters?: boolean; // Optional prop to control multiple filter addition
}
...
export function DataTableFilterList<TData>({
    ...
    }, [table.getState(), table.options.columns]);
    ...
    const onFilterRemove = React.useCallback(
        (filterId: string) => {
            ...
            setFilters(updatedFilters); // Use prop setter
    ...
    const onFiltersReset = React.useCallback(() => {
        setFilters([]); // Use prop setter, reset to empty array
        setJoinOperator("and"); // Use prop setter
            ...
            <Popover open={open} onOpenChange={setOpen}>
                <PopoverTrigger asChild>
                    <Button
                        ...
                        Filter
                        {filters.length > 0 && (
                            <Badge className="h-[18.24px] rounded-[3.2px] px-[5.12px] font-mono text-[10.4px] font-normal">
                                {filters.length}
                            </Badge>
                        )}
                    </Button>
                ...
                <PopoverContent
                    ...
                    <div className="flex w-full items-center gap-2">
                        {!(!canAddMultipleFilters && filters.length > 0) && (
                            <Button
                                size="sm"
                                className="rounded"
                                ref={addButtonRef}
                ...
                <Faceted
                    ...
                    <FacetedContent
                        ...
                        <FacetedInput
                            ...
                            placeholder={
                                columnMeta?.placeholder ?? "Search options..."
                            }
                        />
                        <FacetedList>
            ...
            const displayValue =
                filter.operator === "isBetween" && dateValue.length === 2
                    ? `${formatDate(
                      ...
                      )} - ${formatDate(new Date(Number(dateValue[1])))}`
                    : dateValue[0]
