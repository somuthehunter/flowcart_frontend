...
const logger = (function (window: Window, document: Document, env: string) {
    ...
    const typeAwareStringify = (...optionalParams: unknown[]) => {
        let finalString = "";
    ...
    const formatString = (message?: string, properties?: Properties) => {
    ...
    function debug(
        ...
        if (isRemoteLoggingEnabled && !forceConsole) {
            remoteLogger?.logDebugMessage(
                getInterpolatedMessage(message) || "",
                {
                    ...properties,
                    username,
                    url: window.location.href,
